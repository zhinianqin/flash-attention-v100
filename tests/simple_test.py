...
def prepare_case_tensors(
    ...
    k = torch.randn(total_k, num_heads_k, head_dim, dtype=torch.float16, device=device).contiguous()
    v = torch.randn(total_k, num_heads_k, head_dim, dtype=torch.float16, device=device).contiguous()
...
def run_one_case(
    ...
    try:
        ...
        (
            ...
            dropout_p,
            alibi_slopes,
        ) = prepare_case_tensors(cfg, device, cfg.num_heads, cfg.num_heads_k, head_dim)
        ...
        # split-kv 路径应与 num_splits=1 的同算子结果保持接近
        ...
        return CaseResult(cfg=cfg, status="FAIL", detail=f"exception: {str(e)} | tb: {tb}")
...
def print_results_table(results: List[CaseResult]) -> None:
    header = (
        "| id | H/Hk | Sq | Sk | nsplit | causal | dropout | local | alibi | varlen | status | max_diff | mean_diff | detail |"
    )
    sep = "|---:|:----:|---:|---:|------:|:------:|:-------:|:-----:|:-----:|:------:|:------:|--------:|---------:|:-------|"
    print("\n=== 验证结果表 ===")
    print(header)
    ...
    for r in results:
        ...
        c = r.cfg
        print(
            f"| {c.case_id} | {c.num_heads}/{c.num_heads_k} | {c.seqlen_q} | {c.seqlen_k} | {c.split_num_splits} | {bool_mark(c.causal)} | "
...
def build_case_matrix() -> List[CaseConfig]:
    # 固定 seqlen_q=1，并在多个维度上扩展 split-kv decode 场景矩阵。
    seqlen_pairs = [
        (1, 257),
        (1, 384),
        (1, 513),
...
def matrix_summary(cases: List[CaseConfig]) -> str:
    hhk = sorted({(c.num_heads, c.num_heads_k) for c in cases})
    sk = sorted({c.seqlen_k for c in cases})
    ns = sorted({c.split_num_splits for c in cases})
    varlen = sorted({c.use_varlen for c in cases})
    return (
        f"H/Hk={hhk}, Sk={sk}, num_splits={ns}, varlen={varlen}"
    )
...
def main() -> int:
    ...
    print("覆盖维度: splitkv decode 触发场景（seqlen_q=1, num_heads>num_heads_k, local/alibi/dropout 关闭）")
